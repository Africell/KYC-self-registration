  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${side === "front" ? "bg-cyan-500 text-slate-950" : "bg-violet-500 text-white"}`}>
          {side === "front" ? "1" : "2"}
        </span>
        <h3 className="text-sm font-semibold text-slate-200">{meta.heading}</h3>
        {image && (
        ...
        <label className="group flex cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-slate-700 bg-slate-900/40 px-6 py-10 text-center transition-colors hover:border-cyan-500/50 hover:bg-slate-900/60">
          ...
          <div>
            <p className="text-sm font-medium text-slate-200 group-hover:text-cyan-300">
              Upload {meta.imageAlt.toLowerCase()} of {docLabel}
            </p>
            <p className="mt-1 text-xs text-slate-500">PNG or JPG — clear, flat, all corners visible</p>
          </div>
      ...
      {image && (
        <div className="grid gap-4 sm:grid-cols-[1fr_220px]">
          <div className="rounded-2xl border border-slate-700 bg-slate-900/60 p-3">
            <p className="mb-2 text-xs font-medium text-slate-400">{meta.imageLabel} — preview</p>
            <img
              src={image}
              alt={meta.imageAlt}
