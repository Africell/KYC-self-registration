  ...
  return (
    <div className="space-y-4">
      ...
      {/* {previewMode === "camera" ? (
        ...
        {/* Separate camera button — capture="environment" keeps the camera inside the
            browser WebView instead of launching the native camera app. The native camera
            app backgrounds the browser tab and iOS may evict (reload) it before the file
            is returned, which is why "Take Photo" from the picker above fails on first try.
            Gallery/file upload above is unaffected (no eviction risk without capture). */}
        <label
          className={`flex items-center md:hidden justify-center gap-2 rounded-2xl border border-dashed px-4 py-3 text-sm font-medium transition-colors ${
            isLoading
              ? "cursor-wait border-slate-600 text-slate-500"
              : "cursor-pointer border-slate-700 bg-slate-900/40 text-slate-300 hover:border-cyan-500/50 hover:text-cyan-300"
          }`}
        >
          ...
          {t("doc_camera")}
          <input type="file" accept="image/*" capture="environment" className="hidden" onChange={onUpload} disabled={isLoading} />
        </label>
      {/* )} */}
