...
// The oval constants were designed at 4:3 (VIDEO_W / VIDEO_H). On a portrait
// container (3:4 on mobile) the SVG's preserveAspectRatio="none" would stretch
// the Y axis ~2×, making the oval look elongated. We correct this by scaling
// GUIDE_RY by the ratio of the container's live aspect to the design aspect.
const DESIGN_ASPECT = VIDEO_W / VIDEO_H;
...
export function FaceOvalOverlay({
  ...
  const [aspectRatio, setAspectRatio] = useState(DESIGN_ASPECT);
  ...
  const rX = GUIDE_RX * mobileScale;
  const arcCirc = 2 * Math.PI * rX;
  // Scale GUIDE_RY so the oval maintains the same visual proportions regardless
  // of whether the container is portrait (mobile) or landscape (desktop).
  const rY = GUIDE_RY * (aspectRatio / DESIGN_ASPECT) * mobileScale;

  const tickMarks: [number, number, number, number][] = [
    [GUIDE_CX, GUIDE_CY - rY - 1.5, GUIDE_CX, GUIDE_CY - rY + 2.5],
    [GUIDE_CX, GUIDE_CY + rY - 2.5, GUIDE_CX, GUIDE_CY + rY + 1.5],
    [GUIDE_CX - rX - 1.5, GUIDE_CY, GUIDE_CX - rX + 2.5, GUIDE_CY],
    [GUIDE_CX + rX - 2.5, GUIDE_CY, GUIDE_CX + rX + 1.5, GUIDE_CY],
  ];
  ...
  const arcDash = Math.max(0, Math.min(1, countdown / 3)) * arcCirc;
